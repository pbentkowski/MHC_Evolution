...
# -*- coding: utf-8 -*-
"""
Read a file with genome size histograms "HostGeneNumbTotal_ChrOne.csv" and
a general host statistics file "HostsGeneDivers.csv" to render an animation
how the size of a hosts' chromosomes evolve.
...
ax1.axis([0, GenerData[:, 0].max(), 0, 7])
ax1.set_ylabel('number of all\n' + r'genes$ \times\/10^5 $', fontsize=FontSize)
ax1.set_xlabel('time (host generations)', fontsize=FontSize)
ax1.plot(GenerData[:, 0], GenerData[:, 2]/scale, 'k-')
ax1.grid(True)
...
def animate(i):
    line1.set_data(GenerData[i, 0], GenerData[i, 2]/scale)
    ax2.cla()
    ...
    plt.hist(genMeans[i, 1::], bins=binz, color=(0.3, 0.3, 0.3, 1.0),
             edgecolor="none")
    plt.vlines(100, 0, 300, color="b", lw=2)
    plt.ylim(ymax=300)
    plt.xlim(xmax=MAXX)
...
ani.save('genome_evol.mp4', fps=30)
plt.show()
