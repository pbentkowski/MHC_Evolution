...
def plotAllAllesInPop(meanResult, x_label):
...
#        plt.plot(ww[:, 1], ww[:, 2], 'o', lw=2, ms=10)
        plt.errorbar(ww[:, 1], ww[:, 2], ww[:, 3], lw=2, ms=13)
        plt.annotate(str(var), xy=(ww[-1, 1], ww[-1, 2]))
    plt.xlabel(str(x_label))
    plt.ylabel("mean number of unique MHC alleles in population")
    plt.xlim(xmin=0)
    plt.ylim(ymin=0)
    plt.grid(True)
    ...
    for var in ll:
        ww = meanResult[meanResult[:, 0] == var]
#        plt.plot(ww[:, 1], ww[:, 2], 'o', lw=2, ms=10)
        plt.errorbar(ww[:, 1], ww[:, 4], ww[:, 5], lw=2, ms=13)
        plt.annotate(str(var), xy=(ww[-1, 1], ww[-1, 4]))
    plt.xlabel(str(x_label))
    plt.ylabel("average number of unique MHC alleles in one chromosome")
    plt.xlim(xmin=0)
    plt.ylim(ymin=0)
    plt.grid(True)
#    plt.show()
...
def plotDotMeans(theData):
    ...
    for var in ll:
        ...
        ww = ww[ww['VARX'] == var[1]]
        plt.plot(ww['meanAllel'], ww['indvMean'], clrs[k])
        k += 1
