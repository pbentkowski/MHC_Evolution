...
// === Check if the entered parameters make sense ===
    int numbOfArgs = 14; // how many arguments we need to run this model
    if (argc < numbOfArgs) {
        std::cout << std::endl;
    ...
    int rndSeed, geneLength, exposedMatch, hostPopSize, pathoPopSize, patho_sp, 
        ...
        HeteroHomo;
    double hostMutationProb, pathoMutationProb;
    ...
    if (Data2file.checkParamsIfWrong(rndSeed, geneLength, exposedMatch, hostPopSize, 
            ...
            hostMutationProb, pathoMutationProb, HeteroHomo)){
        std::cout << std::endl;
...
// ======================================
    ...
    ENV.setHostPopulation(hostPopSize, geneLength, hostGeneNumbb);
