...
//    Environment(const Environment& orig);
    virtual ~Environment();
    void setNoMutsVector(int numb_of_species, unsigned long antigen_size, double fixedAntigenFrac, Random &randGen);
    void setNoMutsVecInFours(int numb_of_species, int antigen_size, double fixedAntigenFrac, Random &randGen);
    ...
    void setHostRandomPopulation(int pop_size, unsigned long gene_size, unsigned long chrom_size_lower,
                                 unsigned long chrom_size_uper, int timeStamp, Random &randGen, Tagging_system &tag);
    ...
    void setPathoPopulationFourClades(int pop_size, unsigned long antigenSize, int chrom_size,
        int numb_of_species, unsigned long mhcSize, int timeStamp, double fixedAntigenFrac);
    void infectOneFromOneSpecHetero(Random &randGen);
    ...
    void selectAndReprodHostsAddOffspring(Random &randGen);
    void selectAndReprodHostsReplace(Random &randGen);
    void selectAndReprodHostsNoMating(Random &randGen);
    void selectAndReproducePathoFlexPopSizes(Random &randGen);
    void selectAndReproducePathoFixedPopSizes(Random &randGen);
    ...
    void mutatePathogensWithRestric(double mut_probabl,  unsigned long mhcSize, int timeStamp,
                                    Random &randGen, Tagging_system &tag);
    void mutateHosts(double mut_probabl, int timeStamp, Random &randGen, Tagging_system &tag);
    void mutateHostsWithDelDupl(double mut_probabl, double del, double dupl, 
        unsigned long maxGene, int timeStamp, Random &randGen, Tagging_system &tag);
    void mutateHostsWithDelDuplPointMuts(double mut_probabl, double del, 
        double dupl, unsigned long maxGene, int timeStamp, Random &randGen, Tagging_system &tag);
    double MMtoPMscaling(double MM_prob_mut, unsigned long geneLength);

    void matingWithNoCommonMHCsmallSubset(unsigned long matingPartnerNumber, Random &randGen);
    void matingWithOneDifferentMHCsmallSubset(int matingPartnerNumber, Random &randGen);
    void matingMeanOptimalNumberMHCsmallSubset(int matingPartnerNumber, Random &randGen);
    void matingMaxDifferentMHCs(int matingPartnerNumber, Random &randGen);
